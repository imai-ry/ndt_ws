...
static void points_image_callback(const sensor_msgs::PointCloud2::ConstPtr& input,
                                  const sensor_msgs::Image::ConstPtr& input_img_msg)
{
  double r;
  pcl::PointXYZI p;
  ...
  // color
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr colored_scan_ptr(new pcl::PointCloud<pcl::PointXYZRGB>());
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr transformed_scan_ptr(new pcl::PointCloud<pcl::PointXYZRGB>());
  colorPointCloud(*scan_ptr, input_img_msg, tf_ltoc, *colored_scan_ptr);
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "ndt_color_mapping");
